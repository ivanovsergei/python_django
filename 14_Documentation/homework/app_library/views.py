...
class BookListMixin(ListModelMixin, CreateModelMixin, GenericAPIView):
    """Предоставление для получения списка книг и создания книги."""
...
class AuthorListMixin(ListModelMixin, CreateModelMixin, GenericAPIView):
    ...
    """Предоставление для получения списка автором и создания автора."""

    def get_queryset(self):
        queryset = Author.objects.all()
        book_author = self.request.query_params.get('author')
        if book_author:
            queryset = queryset.filter(first_name=book_author)
        return queryset
